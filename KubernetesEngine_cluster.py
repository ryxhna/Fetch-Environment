...
def gkeCluster(project_id):
    """Fetch GKE clusters based on project ID."""
    ...
    try:
        ...
        for cluster in response.clusters:
            cluster_info = {
                ...
                "End of Standard Support": format_date(cluster.end_of_life.date) if hasattr(cluster, 'end_of_life') else "N/A",
                ...
                "Gateway API": "Enabled" if hasattr(cluster, "gateway_api_config") and cluster.gateway_api_config.enabled else "Disabled",
                "Multi-networking": "Enabled" if hasattr(cluster, "multi_networking") and cluster.multi_networking else "Disabled",
                "Binary Authorization": "Enabled" if cluster.binary_authorization.enabled else "Disabled",
                "Secret Manager": "Enabled" if hasattr(cluster, "secret_manager_config") and cluster.secret_manager_config.enabled else "Disabled",
                "Legacy Authorization": "Enabled" if cluster.legacy_abac.enabled else "Disabled",
                "Security Posture": "Enabled" if hasattr(cluster, "security_posture") and cluster.security_posture.enabled else "Disabled",
